...
class MainFrame(wx.Frame):
    def __init__(self,parent,id):
        wx.Frame.__init__(self,parent,-1,'炜柒运算',size=(200,350))
        #创建，面板
        panel=wx.Panel(self)
        #在面板上添加控件
        self.usinamelabel= wx.StaticText(panel, label="炜柒:", pos=(30, 30))
        self.correct_rate= wx.StaticText(panel, label="成功率", pos=(100, 30))
        prac=wx.Button(panel,label="练习模式",pos=(50,70))
        exam=wx.Button(panel,label="测试模式",pos=(50,120))
        rew = wx.Button(panel, label="错题本", pos=(50, 170))
        reh = wx.Button(panel, label="历史记录", pos=(50, 220))
